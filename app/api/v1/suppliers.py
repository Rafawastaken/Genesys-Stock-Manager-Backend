...
@router.get("", response_model=SupplierList, dependencies=[Depends(require_access_token)])
def list_suppliers(
...
@router.get(
    "/{id_supplier}", response_model=SupplierDetailOut, dependencies=[Depends(require_access_token)]
)
...
def update_supplier_bundle(
    id_supplier: int = Path(..., ge=1), payload: SupplierBundleUpdate = ..., uow: UowDep = None
):
