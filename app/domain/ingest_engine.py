...
# cond ops
def _op_eq(a, b):   fa, fb = _to_float(a), _to_float(b); return (fa==fb) if (fa is not None and fb is not None) else ((clean_text(a) or "")==(clean_text(b) or ""))
def _op_ne(a, b):   return not _op_eq(a, b)
def _op_gt(a, b):   fa, fb = _to_float(a), _to_float(b); return fa is not None and fb is not None and fa >  fb
def _op_gte(a, b):  fa, fb = _to_float(a), _to_float(b); return fa is not None and fb is not None and fa >= fb
def _op_lt(a, b):   fa, fb = _to_float(a), _to_float(b); return fa is not None and fb is not None and fa <  fb
def _op_lte(a, b):  fa, fb = _to_float(a), _to_float(b); return fa is not None and fb is not None and fa <= fb
def _op_contains(a,b): return str(b) in str(a)
def _op_startswith(a,b): return str(a).startswith(str(b))
def _op_endswith(a,b): return str(a).endswith(str(b))
def _op_regex(a,pat): 
    try: return re.search(str(pat), str(a)) is not None
    except re.error: return False
def _op_in(a, coll):
    if isinstance(coll, list): return str(a) in [str(x) for x in coll]
    return str(a) in [s.strip() for s in str(coll).split(",")]

_OPS = {"eq":_op_eq,"ne":_op_ne,"gt":_op_gt,"gte":_op_gte,"lt":_op_lt,"lte":_op_lte,"contains":_op_contains,"startswith":_op_startswith,"endswith":_op_endswith,"regex":_op_regex,"in":_op_in}

class IngestEngine:
    def __init__(self, profile: Optional[JSON]):
        ...
        for k, v in self.fields_cfg.items():
            if isinstance(v, dict):
                ...
                if src: self._field_sources[k] = src

        req = [k for k,v in self.fields_cfg.items() if isinstance(v, dict) and v.get("required") is True]
        if isinstance(self.profile.get("required"), list):
            for k in self.profile["required"]:
                if k not in req: req.append(k)
        ...
        self.row_selector: Optional[JSON] = self.profile.get("row_selector")
        self.defaults: JSON = dict(self.profile.get("defaults") or {})

    def _resolve_token(self, token: Any, mapped: JSON, raw: JSON) -> Any:
        if not _is_ref(token): return token
        name = str(token)[1:]
        if name in mapped: return mapped.get(name)
        if name in raw:    return raw.get(name)
    ...
    def _eval_condition(self, cond: JSON, mapped: JSON, raw: JSON) -> bool:
        if not isinstance(cond, dict) or not cond: return False
        if "and" in cond:
        ...
        fn = _OPS.get(op)
        if not fn or not isinstance(args, list) or len(args)!=2: return False
    ...
    def _apply_defaults(self, mapped: JSON) -> None:
        for k, v in self.defaults.items():
            if _empty(mapped.get(k)): mapped[k] = v

    def _apply_field_transforms(self, target: str, cfg: JSON, mapped: JSON, raw: JSON) -> None:
        if not isinstance(cfg, dict): return
        if target not in mapped and not (cfg.get("source") or cfg.get("from")): return
        val = mapped.get(target)
        if cfg.get("trim") and isinstance(val, str): val = val.strip()
        if cfg.get("lowercase") and isinstance(val, str): val = val.lower()
        if cfg.get("uppercase") and isinstance(val, str): val = val.upper()
    ...
    def _apply_global_rules(self, mapped: JSON, raw: JSON) -> None:
        for rule in self.rules:
            ...
            if not isinstance(conds, list): conds = [conds]
            if not all(self._eval_condition(c, mapped, raw) for c in conds): continue
            set_ops = rule.get("set") or {}
            for k, v in set_ops.items():
                key = self._normalize_set_key(k)
                mapped[key] = self._resolve_token(v, mapped, raw)

    def map_row(self, raw: JSON) -> Tuple[Optional[JSON], Optional[str]]:
        mapped: JSON = {}
        for target, cfg in self.fields_cfg.items():
            if isinstance(cfg, dict):
                src = cfg.get("source") or cfg.get("from")
                if src: mapped[target] = raw.get(src)
        self._apply_defaults(mapped)
        for target, cfg in self.fields_cfg.items():
            if isinstance(cfg, dict): self._apply_field_transforms(target, cfg, mapped, raw)
        ...
        if mapped.get("gtin")  is not None:  mapped["gtin"]  = (clean_text(mapped["gtin"]) or "").strip()
        if mapped.get("price") is not None:  mapped["price"] = (to_decimal_str(mapped["price"]) or "").strip()
        if mapped.get("stock") is not None:
            s = to_int(mapped["stock"]); mapped["stock"] = max(0, s) if s is not None else 0
        return mapped, None
